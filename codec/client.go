...
type clientCodec struct {
	...
	response   header.ResponseHeader // rpc reponse header
...
func (c *clientCodec) WriteRequest(r *rpc.Request, param interface{}) error {
	...
	// put back to requestHeader pool
	defer func() {
		h.ResetHeader()
	...
	// write request header to io stream
	if err := sendFrame(c.w, h.Marshal()); err != nil {
		return err
	}
	// write request body  to io stream
	if err := write(c.w, compressedReqBody); err != nil {
		return err
	}
	if err = c.w.(*bufio.Writer).Flush(); err != nil {
		return err
	}
	return nil
}

// ReadResponseHeader read the rpc response header from io stream
...
// ReadResponseBody read the rpc response body from io stream
